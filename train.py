...
from models.ST_CGAN import Generator, Discriminator,Generator_S
...
# 配置日志
logging.basicConfig(filename='checkpoints/training.log', level=logging.INFO,
...
def get_parser():
    ...
    parser.add_argument('-cs', '--crop_size', type=int, default=224)
...
def train_model(G1, G2, D1, D2, dataloader, val_dataset, num_epochs, parser, save_model_name='model'):
    ...
    g_losses = []
    d_losses = []
    start_epoch = int(parser.load) if parser.load is not None else 0
    for epoch in range(start_epoch,num_epochs+1):
        ...
        logging.info('-----------')
        logging.info(f'epoch {epoch} || Epoch_D_Loss:{epoch_d_loss/batch_size:.4f} || Epoch_G_Loss:{epoch_g_loss/batch_size:.4f}')
        logging.info(f'timer: {t_epoch_finish - t_epoch_start:.4f} sec.')
...
def main(parser):
    ...
    if parser.load is not None:
        print(parser.load)
        logging.info(f'load checkpoint {parser.load}')
    ...
    train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True,num_workers=16) #num_workers=4
